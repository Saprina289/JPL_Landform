...
def download_data(url , output_folder): #Scraper
    ...
    # Exclude everything except for those starting with NLF and NRF.
    #data = soup.find_all('NRF')
...
#commented out bands==1 to prevent black/white images from downloading
    ...
    for filename in os.listdir(INPUTDIR): #for every IMG img in the given filename path
        ...
        with open(INPUTDIR+'/'+filename,'rb') as f:
            ...
            while continuing == 1:
                ...
                #     # focal_len = s.split(" ")[1]  #paper says focal_len is constant
                #     focal_len = 14.67
            ...
            pixel_size = 0.33/(float(focal_len)) #using formula to calculate pixel size for given pixel scale and focal length

            x0 = (pixel_size) * (hc - ( hlines / 2))

            y0 = - (pixel_size) * (vc - (hline_samples/ 2))
...
if __name__ == "__main__":
    ...
    # print()
    # cam = "ncam"
    # sol = input("Enter Sol \n Pass the right format 89 --> 0089 (zcam callib) 89 --> 00089 (other cams) \n")
    # url = build_url(cam,sol) #creates the url based on inputs given
    # print(url)


    # for sol_num in range(52,62):
            
        # if(sol_num == 13 or sol_num == 14 or sol_num == 18 or sol_num == 19 or sol_num == 20 or sol_num == 21):
    #         continue
    ...
    root_path = os.getcwd() 
    new_sol_folder_path = os.path.join(root_path, "sol" + sol + "_NAVcam")
